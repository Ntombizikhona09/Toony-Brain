...
def toggle_inputs():
    ...
    if input_visible:
        ...
        # Move Start button below input fields when visible
...
def create_ui():
    ...
    welcome_label = Label(main_frame, text="Welcome to Toony Brain!", font=(
        "Arial", 24), bg='purple', fg='white')
    welcome_label.pack(pady=20)

    # ✅ Profile Label (Lowered)
    choose_label = Label(main_frame, text="Choose a profile:",
                         font=("Arial", 16), bg='purple', fg='white')
    ...
    try:
        smiley_img = Image.open(
            "assets/spunge.png").resize((130, 130), Image.Resampling.LANCZOS)
    ...
    try:
        hello_kitty_img = Image.open(
            "assets/hello-kitty-wallpaper-37_605.webp").resize((130, 130), Image.Resampling.LANCZOS)
        hello_kitty_img = ImageTk.PhotoImage(hello_kitty_img)
    except Exception as e:
        print(f"Error loading Hello Kitty image: {e}")
        hello_kitty_img = None

    smiley_button = Button(profile_frame, image=smiley_img, bg='yellow', command=toggle_inputs) if smiley_img else Button(
        profile_frame, text="Spunge", command=toggle_inputs)
    smiley_button.grid(row=0, column=0, padx=20)

    hello_kitty_button = Button(profile_frame, image=hello_kitty_img, bg='pink', command=toggle_inputs) if hello_kitty_img else Button(
        profile_frame, text="Hello Kitty", command=toggle_inputs)
    ...
    Label(input_frame, text="Name :", font=("Arial", 14), bg='purple',
          fg='white').grid(row=0, column=0, padx=10, pady=5)
    name_entry = Entry(input_frame, font=("Arial", 14))
    name_entry.grid(row=0, column=1, padx=10, pady=5)

    Label(input_frame, text="Surname :", font=("Arial", 14),
          bg='purple', fg='white').grid(row=1, column=0, padx=10, pady=5)
    surname_entry = Entry(input_frame, font=("Arial", 14))
    surname_entry.grid(row=1, column=1, padx=10, pady=5)

    Label(input_frame, text="Date of Birth :", font=("Arial", 14),
          bg='purple', fg='white').grid(row=2, column=0, padx=10, pady=5)
    ...
    start_button = Button(main_frame, text="Start", font=(
        "Arial", 14), bg='green', fg="white")
